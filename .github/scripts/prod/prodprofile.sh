...
function find_sleep_profile()
{
    ...
    RESPONSE_CODE=$(curl -k -s -o /prod/null -w "%{http_code}" https://localhost/profile)

    if [[ ${RESPONSE_CODE} -ge 400 ]] # 400 보다 크면 (즉, 40x/50x 에러 모두 포함)
    then
        CURRENT_PROFILE=prod-2 # 정상적으로 수행 중이지 않을 경우
    else
        CURRENT_PROFILE=$(curl -k -s https://localhost/profile)
    fi

    if [[ ${CURRENT_PROFILE} == prod-1 ]]
    then
      IDLE_PROFILE=prod-2 # 엔진엑스랑 연결되지 않은 profile
    else
      IDLE_PROFILE=prod-1
    fi
